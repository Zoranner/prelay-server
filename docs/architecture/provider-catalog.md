...
供应商需要经代理访问上游时，在条目上加 `proxy_url`（只支持 `http`/`https`，留空表示直连）：
...
代理属于目录配置，重启后生效；该供应商的协议转发、连通性检查、协议测试和模型发现都会经过这个代理，同一个代理地址共用一个 HTTP 客户端连接池。

### 模型清单

目录条目回答"这个供应商类型可以提供哪些模型"，供应商记录回答"这家供应商启用了哪些模型"。供应商创建时缺省启用目录条目的全部模型，客户端用目录条目作为可选清单让用户逐个选择；之后两者不再自动同步：

- 目录新增模型不会自动生效，需要在供应商编辑页启用。
- 目录移除模型不影响已保存的供应商，存量接入点继续可用；但再次保存该供应商时，清单里已不在目录条目中的模型必须先移除，否则拒绝保存并指出具体模型。
- 供应商类型整体下架后，该供应商只读：现有模型继续可用，不能再编辑，只能删除。

接入点保存校验、`/v1/models` 与请求解析都按供应商自己的清单判定，因此清单之外的模型既不能被配置，也不会被返回或转发。客户端的编辑界面把供应商清单与目录条目比对，区分"已启用""未启用""目录已下架"三种状态，只有前两种可以切换。

服务启动时按目录对账接入点模型：供应商记录已不存在、或模型不在该供应商的清单里时，删除对应的接入点模型与候选路由；供应商上游名变化时，按目录映射回填接入点保存的上游名。
