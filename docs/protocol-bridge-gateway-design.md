...
src/routes/v1/responses.rs
src/routes/v1/messages.rs
src/routes/v1/models.rs
```

`routes/management/` 仅供桌面客户端调用，负责按设备凭据确定当前身份；`routes/v1/` 仅提供正式协议入口。根路径没有透明代理，`/proxy` 不存在。
