...
Compose 固定拉取 `ghcr.io/zoranner/prelay-server:0.1.4`，并以只读方式挂载 `app/config/`。模型价格是可选配置；可复制 `config/model_prices.example.json` 为 `app/config/model_prices.json` 后调整内容。
