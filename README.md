...
使用该 Compose 文件时，客户端安装包的容器内目录是 `/app/updates`，对应宿主机的 `deploy/updates`；`deploy/data` 仅用于 SQLite 数据库。GitHub 不可用时，可将发布产物原文件直接放入对应目标和版本目录，例如 `deploy/updates/windows/x64/0.2.0/Prelay_0.2.0_x64-setup.exe`。
...
Compose 固定拉取 `ghcr.io/zoranner/prelay-server:0.3.3`，并以只读方式挂载 `app/config/catalog/`；目录内必须包含 `providers.toml` 和 `models/` 下的两类模型文件，语言模型的基础指令模板放在同一目录的 `models/instructions/`。
