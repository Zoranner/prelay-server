...
Compose 固定拉取 `ghcr.io/zoranner/prelay-server:0.3.2`，并以只读方式挂载 `app/config/catalog/`；目录内必须包含 `providers.toml` 和 `models/` 下的两类模型文件，语言模型的基础指令模板放在同一目录的 `models/instructions/`。
