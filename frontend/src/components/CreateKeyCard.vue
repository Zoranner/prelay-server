<template>
  <div class="px-6 py-5 space-y-4">
    ...
    <div>
      <label class="block text-xs font-medium text-stone-500 mb-1.5 uppercase tracking-wide">上游 API Key</label>
      <input
    ...
    <p v-if="error" class="text-sm text-[#a83232] bg-[#fdf3f3] border border-[#ecc8c8] rounded-lg px-3 py-2">
      {{ error }}
    </p>
  ...
  <div v-if="createdToken" class="px-6 pb-5">
    <div class="bg-[#f2f8f5] border border-[#9fc9b2] rounded-xl p-4">
      ...
      <div class="flex items-center gap-2">
        <code class="flex-1 bg-white border border-[#9fc9b2] rounded-lg px-3 py-2 text-xs font-mono text-stone-700 break-all select-all">{{ createdToken }}</code>
        <button
...
const { form, creating, error, createdToken, copied, onProviderChange, submit, copyToken } = useCreateKey();
</script>
