...
`prelay-server` 是 Rust/Axum 协议桥接服务：按身份隔离配置，以 `PRELAY_MASTER_KEY` 加密保存 Provider API Key，执行上游调用，并暴露管理 API 与 `/v1` 协议入口。
...
- 失败重试、候选切换、超时和最多候选数是服务端部署策略，通过 `PRELAY_UPSTREAM_*` 环境变量配置；不要将它们变成桌面端的终端用户设置。
...
- 本地运行需要有效的 `PRELAY_MASTER_KEY`；可由仓库根 `.env` 加载。默认监听 `0.0.0.0:18080`，`LISTEN_PORT` 可覆盖。
- 修改 Rust 代码后在仓库根目录执行：
