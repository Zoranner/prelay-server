...
const allUpstreamProtocols: UpstreamProtocol[] = [
  "responses",
  "openai",
  "anthropic",
];
...
watch(
  ...
  (provider) => {
    ...
    apiKey.value = "";
    modelsText.value =
      provider?.models.map((model) => model.model_name).join("\n") ?? "";
    const capabilities = provider?.capabilities;
    upstreamProtocols.value = (capabilities?.upstream_protocols ?? []).filter(
      isUpstreamProtocol,
    );
    protocolBaseUrls.responses =
      capabilities?.protocol_base_urls?.responses ?? "";
    protocolBaseUrls.openai = capabilities?.protocol_base_urls?.openai ?? "";
    protocolBaseUrls.anthropic =
      capabilities?.protocol_base_urls?.anthropic ?? "";
...
<template>
  <form class="space-y-5" @submit.prevent="submit">
    ...
    <fieldset class="space-y-4 rounded-lg border border-stone-200 p-4">
      <legend class="px-1 text-sm font-semibold text-stone-800">
        上游能力覆盖
      </legend>
      <div class="grid gap-3 sm:grid-cols-3">
        <label
          v-for="protocol in allUpstreamProtocols"
          :key="protocol"
      ...
      <div class="grid gap-3 sm:grid-cols-2">
        <label class="field"
          ...
          ><input v-model.number="maxContextTokens" min="1" type="number"
        /></label>
        <label class="field"
          ><span>最大输出 Token</span
          ><input v-model.number="maxOutputTokens" min="1" type="number"
        /></label>
      </div>
      <div class="grid gap-3 sm:grid-cols-2">
        <label class="flex items-center gap-2 text-sm text-stone-600"
          ><input v-model="toolCalls" type="checkbox" />工具调用</label
        >
        <label class="flex items-center gap-2 text-sm text-stone-600"
          ><input v-model="reasoning" type="checkbox" />推理</label
        >
        <label class="flex items-center gap-2 text-sm text-stone-600"
          ><input v-model="toolChoice" type="checkbox" />工具选择</label
        >
        <label class="flex items-center gap-2 text-sm text-stone-600"
          ><input
        ...
        >
        <label class="flex items-center gap-2 text-sm text-stone-600"
          ><input v-model="systemMessages" type="checkbox" />系统消息</label
        >
        <label class="flex items-center gap-2 text-sm text-stone-600"
          ><input
            v-model="structuredOutputs"
            type="checkbox"
          />结构化输出</label
        >
        <label class="flex items-center gap-2 text-sm text-stone-600"
          ><input v-model="streamingUsage" type="checkbox" />流式用量</label
        >
      </div>
    </fieldset>
