...
function ensureMappingSelection(model: InterfaceModel) {
  const provider =
    availableProviders.value.find((item) => item.id === model.provider_id) ??
    availableProviders.value[0];
  if (!provider) return false;

  model.provider_id = provider.id;
  if (
    !provider.models.some((item) => item.model_name === model.upstream_model)
  ) {
  ...
  <form class="space-y-5" @submit.prevent="submit">
    ...
    <section class="space-y-3">
      <div class="flex items-center justify-between gap-3">
        <h3 class="text-sm font-semibold text-stone-800">模型映射</h3>
        <button
      ...
      <p v-if="!availableProviders.length" class="text-sm text-amber-700">
        请先配置包含模型的供应商。
      </p>
      <div
        v-for="(model, index) in models"
        :key="`${model.provider_id}-${index}`"
        class="grid gap-2 rounded-lg border border-stone-200 bg-stone-50 p-3 sm:grid-cols-[1fr_1fr_1fr_auto]"
      >
        <select
          v-model="model.provider_id"
          class="table-control"
          aria-label="供应商"
          @change="ensureMappingSelection(model)"
        >
          <option
        ...
        <select
          v-model="model.upstream_model"
          class="table-control"
          required
          aria-label="上游模型"
        >
          <option
            v-for="upstream in modelsForProvider(model.provider_id)"
            :key="upstream.id"
            :value="upstream.model_name"
          >
            {{ upstream.model_name }}
          </option>
        </select>
